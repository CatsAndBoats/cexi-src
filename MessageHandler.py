...
# Mapping of zone IDs to zone names
ZONE_ID_TO_NAME = {
    0: "unknown",
    1: "Phanauet_Channel",
    ...
    273: "Woh_Gates",
    274: "Outer_RaKaznar",
    275: "Outer_RaKaznar_[U1]",
    276: "RaKaznar_Inner_Court",
    277: "RaKaznar_Turris",
    278: "Gwora-Corridor",
    279: "Walk_of_Echoes_[P2]",
    280: "Mog_Garden",
    281: "Leafallia",
    282: "Mount_Kamihr",
    283: "Silver_Knife",
    ...
    294: "Dynamis-San_dOria_[D]",
    295: "Dynamis-Bastok_[D]",
    296: "Dynamis-Windurst_[D]",
    297: "Dynamis-Jeuno_[D]",
    298: "Walk_of_Echoes_[P1]",
    299: "Gwora-Throne_Room",
}

def get_message_type_label(message_type):
    return MESSAGE_TYPE_LABELS.get(message_type, f"UNKNOWN({message_type})")

def get_zone_name(zone_id):
    return ZONE_ID_TO_NAME.get(zone_id, f"ZoneID({zone_id})")
...
def main():
    ...
    print("✅ ChatLogger: Listening on tcp://127.0.0.1:5556...\n")

    while True:
        try:
            msg = socket.recv()
            json_str = msg.decode("shift_jis", errors="replace")
            ...
            message_type = data.get("messageType", -1)
            message_type_label = get_message_type_label(message_type)

            print(f"[CHAT] {message_type_label} | {zone_name} | {sender}: {message}")

        except json.JSONDecodeError as e:
            print(f"⚠️ JSON error: {e}")
            print(f"⚠️ Raw decoded string: {json_str}")
